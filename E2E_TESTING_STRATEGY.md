...
| Add multiple pizza types | Cart shows each selected pizza with the correct quantity |
| Cart reflects quantities | Total item count in cart updates correctly |
| Cart total updates correctly | Price total matches selected items and quantities |
...
| Rejects whitespace-only name | Validation when name contains only spaces |
...
| Confirmation shows correct details | Receipt includes customer name, selected pizzas, quantities, and total |

### 3. `order-lookup.spec.ts` - Order Management

| Test | What to verify |
|------|---------------|
| Look up a valid order ID | Returns the correct order details |
| Look up an invalid/unknown ID | Shows an appropriate error/not-found message |
| Empty ID field | Validation message shown |
| Look up newly created order | Place an order first, then search for that returned order ID |

### 4. `ui.spec.ts` - UI & Accessibility

| Test | What to verify |
|------|---------------|
| Dark mode toggle works | Clicking the dark mode button switches theme class on `<body>` |
...
| Form controls are accessible | Inputs and buttons have clear accessible names |
...
- Verify cart total before placing the order
